...
;(function ( $, window, document, undefined ) {
    ...
    $.extend(Plugin.prototype, {
        ...
        buildCalendar: function (fromDate, calendar) {
            ...
            var currPos = 0;
            var lastC = ''
            for(i=0;i<this.settings.takendays.length;i++){
                var c = this.settings.takendays[i]
                if(c != '[' && c != ']' && c!= '\"' && c != ','){
                    if (currPos == 0){
                        currDay += c
                    }
                    ...
                    tdList.push([parseInt(currYear), parseInt(currMonth)-1, parseInt(currDay)])
                    currDay = '';
                    currMonth = '';
                    currYear = '';
                    currPos = 0;
            ...
            for(i=0;i<tdList.length-1;i++){
                ...
                var d = new Date(2000+tdList[i][0],tdList[i][1],tdList[i][2])
                if (takenDateList.indexOf(d > -1)){
                    takenDateList.push(d);
                    console.log(d)
                }
            }
            console.log(takenDateList)
            ...
            for(var day = firstDay; day <= lastDay; day.setDate(day.getDate())) {
                ...
                for(var i = 0; i<7; i++) {
                    ...
                    for(var j=0;j<takenDateList.length;j++){
                        if(day.toDateString() === takenDateList[j].toDateString()){
                            td.find(".day").addClass("complete");
                            break
                        }
                    }
